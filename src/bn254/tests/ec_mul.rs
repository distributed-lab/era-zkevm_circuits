pub mod test {
    ...
    fn test_width_4_multiplication() {
        ...
        for (i, test) in EC_MUL_TEST_CASES.tests.iter().enumerate() {
            ...
            let expected = test.expected.to_projective_point(cs);
            let expected_x = expected.x.witness_hook(cs)().unwrap().get();
            let expected_y = expected.y.witness_hook(cs)().unwrap().get();
